...
/* Three rotating buffers let callers chain calls in one printf, e.g.
 *   nd_log_info("%s -> %s", nd_ntoa(a), nd_ntoa(b));
 * __thread makes the buffers per-thread — no-op today (daemon is single-threaded)
 * but forward-compatible if that ever changes. */
const char *nd_ntoa(const nd_addr_t *addr)
{
    static __thread int index;
    static __thread char buf[3][64];
...
/* Randomized on first use so attackers can't precompute bucket collisions.
 * /dev/urandom is preferred; gettimeofday is a fallback if the read fails. */
static uint32_t ndL_hash_seed;
static bool ndL_hash_seed_initialized;

static void ndL_init_hash_seed(void)
{
    int fd = open("/dev/urandom", O_RDONLY | O_CLOEXEC);
    if (fd >= 0) {
        ssize_t n;
        do {
            n = read(fd, &ndL_hash_seed, sizeof(ndL_hash_seed));
        } while (n < 0 && errno == EINTR);
        close(fd);
        if (n == (ssize_t)sizeof(ndL_hash_seed) && ndL_hash_seed != 0) {
            ndL_hash_seed_initialized = true;
            return;
        }
    }
    ...
    ndL_hash_seed = ((uint32_t)tv.tv_sec * 2654435761u) ^ ((uint32_t)tv.tv_usec * 40503u) ^ (uint32_t)getpid();
    if (ndL_hash_seed == 0)
        ndL_hash_seed = 0x9e3779b9u;
    ndL_hash_seed_initialized = true;
}

uint32_t nd_addr_hash(const nd_addr_t *addr)
{
    if (!ndL_hash_seed_initialized)
        ndL_init_hash_seed();
    uint32_t hash = ndL_hash_seed;
