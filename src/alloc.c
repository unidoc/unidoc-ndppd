...
/* Cap on per-chunk growth. Without a cap, `ndL_alloc_size *= 2` per new chunk
 * grows unboundedly under sustained allocation pressure, and chunks are only
 * released at daemon exit — so peak memory becomes a permanent floor. */
#ifndef NDPPD_CHUNK_ALLOC_MAX
#    define NDPPD_CHUNK_ALLOC_MAX (1U << 20) /* 1 MiB */
#endif
...
/* Controlled exit — logs to stderr because nd_log may not be initialized yet
 * (nd_alloc is used during config parse) and is not async-safe.  Aborts
 * would dump core and leave the supervisor to respawn; exit(1) shuts down
 * cleanly with a clear diagnostic. */
static void ndL_fatal(const char *what, size_t size)
{
    fprintf(stderr, "fatal: nd_alloc %s (size=%zu, max=%d) — refusing to continue\n",
            what, size, NDPPD_MAX_ALLOC);
    exit(EXIT_FAILURE);
}
...
void nd_free(void *ptr, size_t size)
{
    if (size == 0 || size > NDPPD_MAX_ALLOC)
        ndL_fatal("free with invalid size", size);
...
{
    if (size == 0 || size > NDPPD_MAX_ALLOC)
        ndL_fatal("alloc with invalid size", size);
    ...
    if (chunk == NULL) {
        fprintf(stderr, "fatal: nd_alloc out of memory (requested chunk %zu bytes)\n", ndL_alloc_size);
        exit(EXIT_FAILURE);
    }
    ...
    *chunk = (ndL_chunk_t){
    ...
    if (ndL_alloc_size < NDPPD_CHUNK_ALLOC_MAX)
        ndL_alloc_size *= 2;
    if (ndL_alloc_size > NDPPD_CHUNK_ALLOC_MAX)
        ndL_alloc_size = NDPPD_CHUNK_ALLOC_MAX;
