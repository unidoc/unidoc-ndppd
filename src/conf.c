...
static const ndL_cfinfo_t ndL_cfinfo_table[] = {
    ...
    { "promiscuous", NDL_PROXY, NDL_BOOL, offsetof(nd_proxy_t, promiscuous), 0, 0, NULL },
...
__attribute__((format(printf, 2, 3))) static void ndL_error(const ndL_state_t *state, const char *fmt, ...)
{
    char buf[512];
    va_list va;

    va_start(va, fmt);
    /* fmt comes from an internal caller; -Wformat is applied at the callsite via
     * the format attribute above, so the nonliteral warning here is a false
     * positive on the vprintf forwarder pattern. */
...
static bool ndL_accept_name(ndL_state_t *state, char *str, size_t size)
{
    ...
    /* size > 1, not > 0: always keep room for the trailing NUL below.
     * size must be decremented every iteration - this loop used to run
     * on tmp.offset/tmp.length alone, so any identifier longer than the
     * caller's buffer (ifname[IF_NAMESIZE], key[32], buf[16], ...) wrote
     * straight past it. */
    while (size > 1 && tmp.offset < tmp.length) {
        char c = tmp.data[tmp.offset];
    ...
    if (tmp.offset == state->offset)
        return false;

    /* Stopped because the buffer filled up, not because the identifier
     * ended - reject instead of silently accepting a truncated value. */
    if (size <= 1 && tmp.offset < tmp.length) {
        char c = tmp.data[tmp.offset];
        if (isalnum(c) || strchr(valid, c))
            return false;
    }
