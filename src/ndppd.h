...
/* Baked in by the Makefile (VERSION=... or `git describe`) - "dev" here
 * is only the fallback for a hand-invoked cc that skips the Makefile. */
#ifndef NDPPD_VERSION
#    define NDPPD_VERSION "dev"
#endif
...
struct nd_proxy {
    ...
    nd_rule_t *rules;
    bool router;
    bool promiscuous; /* Linux: PROMISC instead of ALLMULTI. FreeBSD: ignored (BPF requires promisc). */
};
...
/* Cap on queued subscribers per session — bounds memory under spoofed-NS floods. */
#ifndef ND_SESSION_MAX_SUBS
#    define ND_SESSION_MAX_SUBS 32
#endif

/* Cap on total live sessions across all proxies — bounds memory when an
 * on-link host sends NS for many distinct targets within a proxied
 * (often wide, e.g. /80 or wider) prefix. Without this, nd_session_create()
 * has no limit at all beyond process OOM. */
#ifndef ND_SESSION_MAX
#    define ND_SESSION_MAX 4096
#endif
...
struct nd_iface {
    ...
    nd_proxy_t *proxy;
    bool promiscuous; /* Which L2 mode was requested at open time — used for symmetric close. */
...
nd_iface_t *nd_iface_open(const char *if_name, unsigned int if_index, bool promiscuous);
...
/*! Reconciles every open iface against the current kernel state.
 *
 * Compares each cached (name, index, lladdr) against `if_nametoindex()` +
 * `SIOCGIFHWADDR`. If either changed — which happens whenever an Incus
 * bridge is deleted+recreated, or a veth is added/removed on a bridge —
 * drops the stale PACKET_MR_ALLMULTI membership, refreshes lladdr, and
 * rejoins on the new ifindex. Called both from netlink LINK events (rt.c)
 * and periodically from the main loop (ndppd.c) as a safety net.
 */
...
void nd_log_printf(nd_loglevel_t level, const char *fmt, ...) __attribute__((format(printf, 2, 3)));
