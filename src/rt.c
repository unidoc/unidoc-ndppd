    ...
    for (;;) {
        ...
        for (struct nlmsghdr *hdr = (struct nlmsghdr *)buf; NLMSG_OK(hdr, len); hdr = NLMSG_NEXT(hdr, len)) {
            ...
            if (hdr->nlmsg_type == RTM_NEWROUTE) {
                ...
                /* Any link add/remove could mean one of our cached ifaces
                 * changed ifindex (bridge recreate, veth churn). Rebind
                 * all iface memberships; O(#proxied ifaces) which is tiny. */
                nd_iface_rebind_all();
            }
        }
    }
...
static void ndL_handle(void *buf, size_t buflen)
{
    for (size_t i = 0; i < buflen;) {
        if (i + sizeof(ndL_msghdr_t) > buflen)
            break;

        ndL_msghdr_t *hdr = (ndL_msghdr_t *)((uint8_t *)buf + i);
        size_t msglen = hdr->msglen;

        /* Zero-length record would loop forever; short record would read past buffer. */
        if (msglen < sizeof(ndL_msghdr_t) || msglen > buflen - i)
            break;
        ...
        /* Any interface state change (up/down, MTU, MAC) or arrival/
         * departure could mean a cached bpf handle is now a black hole.
         * Trigger a full rebind — cheap (O(#proxied ifaces)) and it's
         * the same code path the 5-second safety-net poll uses, so
         * behaviour is uniform whether the event is delivered promptly
         * or not. RTM_IFANNOUNCE is FreeBSD/NetBSD; guard for portability. */
        case RTM_IFINFO:
...
static bool ndL_dump(int type)
{
    ...
    if (!buf) {
        nd_log_error("malloc(%zu) failed", size);
        return false;
    }
...
bool nd_rt_open()
{
    ...
    /* Also subscribe to RTNLGRP_LINK: without it we never learn when an
     * interface is deleted+recreated (typical Incus bridge lifecycle),
     * so the cached ifindex + PACKET_MR_ALLMULTI membership go stale and
     * every proxied NS silently disappears. The handler triggers
     * nd_iface_rebind_all() on any LINK event. */
    struct sockaddr_nl addr = {
        .nl_family = AF_NETLINK,
        .nl_groups = (1 << (RTNLGRP_IPV6_IFADDR - 1))
